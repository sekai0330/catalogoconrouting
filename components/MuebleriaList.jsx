        ...
        <section>
            <ul className="grid grid-cols-2 lg:grid-cols-3 gap-2 md:gap-3 lg:gap-5">
                {muebleria.map(mueble => (
                    <li key={mueble.id} className={`${sansita.className} bg-white flex flex-col justify-between py-3 px-5 rounded-lg border shadow-lg space-y-1`}>
                        <div>
                        ...
                        <Link legacyBehavior href={`/muebles/${mueble.url}`}>
                                <a className="flex items-center gap-2 justify-center bg-amber-700 hover:bg-amber-900 border-2 border-botones transition-colors text-botones text-white mt-5 md:mt-0 py-2 text-lg px-5 rounded-xl">
                                    Ver Detalles
                                </a>
                        </Link>
                    </li>
